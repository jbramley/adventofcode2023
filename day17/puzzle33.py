...
def puzzle33():
    ...
    while True:
        ...
        if n.x > 0 and (0 >= n.dx > -3) and nodes[l_x] is None:
        ...
        if n.x < (grid_x - 1) and (0 <= n.dx < 3) and nodes[r_x] is None:
            if n.y == (grid_y - 1) and (n.x + 1) == (grid_x - 1):
                shortest_path = nodes[n] + int(grid[n.y][n.x + 1])
                break
            nodes[r_x] = nodes[n] + int(grid[n.y][n.x + 1])
            unvisited.setdefault(nodes[n] + int(grid[n.y][n.x + 1]), set())
            unvisited[nodes[n] + int(grid[n.y][n.x + 1])].add(r_x)
        if n.y > 0 and (0 >= n.dy > -3) and nodes[u_y] is None:
        ...
        if n.y < (grid_y - 1) and (0 <= n.dy < 3) and nodes[d_y] is None:
            if (n.y + 1) == (grid_y - 1) and n.x == (grid_x - 1):
